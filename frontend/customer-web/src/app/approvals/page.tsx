          ...
          {items.map(a => (
            <div key={a.id} className="p-4 bg-white rounded-lg border">
              <div className="flex justify-between items-center">
                ...
                <span className={`text-sm px-2 py-0.5 rounded ${a.status === "PENDING" ? "bg-yellow-50 text-yellow-700" : a.status === "APPROVED" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}>{labelFor(a.status)}</span>
              </div>
              <div className="mt-2 text-sm space-y-1">
                <p>类型：<span className="font-semibold">{labelFor(a.approval_type)}</span> | 操作：{labelFor(a.tool_name)}</p>
                <p>风险级别：{labelFor(a.risk_level)}</p>
